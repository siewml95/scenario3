...
public class NodeLink extends AnchorPane {
	...
	public void bindEnds (DraggableNode source, DraggableNode target) {
		...
		node_link.startXProperty().bind(
			Bindings.add(source.layoutXProperty(), ((source.getWidth() / 2.0) + 50)));
		
		node_link.startYProperty().bind(
				Bindings.add(source.layoutYProperty(), ((source.getWidth() / 2.0)  +42)));
		
		node_link.endXProperty().bind(
				Bindings.add(target.layoutXProperty(), ((target.getWidth() / 2.0) + 50)));
		
		node_link.endYProperty().bind(
				Bindings.add(target.layoutYProperty(), ((target.getWidth() / 2.0) + 42)));
