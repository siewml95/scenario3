...
public class NodeLink extends AnchorPane {
	...
	public void bindEnds (DraggableNode source, DraggableNode target) {
		node_link.startXProperty().bind(
			Bindings.add(source.layoutXProperty(), (source.getWidth() / 2.0) + linkOffset ));
		
		node_link.startYProperty().bind(
				Bindings.add(source.layoutYProperty(), (source.getWidth() / 2.0) + linkOffset));
		
		node_link.endXProperty().bind(
				Bindings.add(target.layoutXProperty(), (target.getWidth() / 2.0) + linkOffset));
		
		node_link.endYProperty().bind(
				Bindings.add(target.layoutYProperty(), (target.getWidth() / 2.0) + linkOffset));
		
		source.registerLink (getId());
		target.registerLink (getId());
	}
        public void bindEnds2 (DraggableNode source, DraggableNode target) {
		node_link.startXProperty().bind(
			Bindings.add(source.layoutXProperty(), ((source.getWidth() / 2.0) + 50 + linkOffset)));
		
		node_link.startYProperty().bind(
				Bindings.add(source.layoutYProperty(), ((source.getWidth() / 2.0)  +42 + linkOffset)));
		
		node_link.endXProperty().bind(
				Bindings.add(target.layoutXProperty(), ((target.getWidth() / 2.0) + 50 + linkOffset)));
		
		node_link.endYProperty().bind(
				Bindings.add(target.layoutYProperty(), ((target.getWidth() / 2.0) + 42 + linkOffset)));
          ...
          public double randomOffset()
        {
            ...
            double sign = offsetGenerator.nextDouble();
            if(sign > 0.5)
                return offsetGenerator.nextDouble() * 30.0;
            else
                return -(offsetGenerator.nextDouble() * 30.0);
        ...
        public void setLinkOffset(double offset)
        {
            linkOffset = offset;
        }
